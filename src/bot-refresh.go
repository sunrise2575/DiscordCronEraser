...
func treatRefresh(sess *discordgo.Session, msg *discordgo.Message) {
	channel, e := sess.Channel(msg.ChannelID)
	if e != nil {
		log.Println(e)
		return
	}
	...
	dbTx(db, func(tx *sql.Tx) bool {
		...
		if affected > 0 {
			log.Printf("delete %v records at treatRefresh by user '%v' (user ID: %v, channel ID: %v)",
